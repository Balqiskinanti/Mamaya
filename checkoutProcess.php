...
if($_POST) //Post Data received from Shopping cart page.
{
	...
	foreach($_SESSION['Items'] as $key=>$item) {
		...
		if($qtyPurchase > $qtyAvailable){
			echo "Product #$pid : $name is Out of Stock! <br />";
			echo "Please return to shopping cart to make amendments to your purchase";
			include("footer.php");
			exit;
		}
	}
...
{	
	   ...
	   "SUCCESSWITHWARNING" == strtoupper($httpParsedResponseAr["ACK"])) 
	{
		...
		{
			$pid = $item["productId"];
			$minusQty = $item["quantity"];

			$qry = "UPDATE product SET Quantity=(SELECT Quantity from product WHERE ProductID=?) - ? WHERE ProductID=?";
			$stmt = $conn->prepare($qry);
			$stmt->bind_param("iii",$pid, $minusQty, $pid);
			$stmt->execute();
			$stmt->close();
		}
